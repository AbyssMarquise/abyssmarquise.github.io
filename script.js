...
async function loadPages() {
  const pages = await Promise.all(PAGE_FILES.map((name) => fetchJson(`content/${name}.json`)));
...
async function collectionPaths() {
  ...
  const paths = tree.filter((item) => item.type === "blob" && item.path.endsWith(".json")).map((item) => item.path);
  return {
...
function navigationButtons(targets) {
  ...
  [["previous", "m15 18-6-6 6-6"], ["next", "m9 6 6 6-6 6"]].forEach(([direction, path]) => {
    const button = document.createElement("button");
    button.className = `project-switch project-switch--${direction}`;
    button.type = "button";
    button.dataset.projectSwitch = targets[direction];
    button.setAttribute("aria-label", direction === "previous" ? "Afficher la miniature précédente" : "Afficher la miniature suivante");
...
function buildSlide(layout, projects, categoryIndex, slideIndex, slideCount) {
  const templateName = layout === "posters"
  ...
  const slide = document.querySelector(`#${templateName}`).content.firstElementChild.cloneNode(true);
  const slideId = `${categoryIndex + 1}-${slideIndex + 1}`;
  slide.dataset.projectSlide = slideId;
  slide.hidden = slideIndex > 0;
  slide.querySelector(".project-number").textContent = String(categoryIndex + 1).padStart(2, "0");
  ...
  if (layout === "posters") {
    projects.slice(0, 2).forEach((project, index) => {
      assets.appendChild(lightboxLink(project, `poster poster-${index === 0 ? "one" : "two"}`, gallery));
    });
  } else if (layout === "browser") {
    projects.forEach((project) => assets.appendChild(lightboxLink(project, "browser-ad", gallery)));
    slide.querySelector(".browser-logo").textContent = projects[0]?.browserLogo || "MÉDIAS";
    slide.querySelector(".browser-category").textContent = projects[0]?.browserCategory || projects[0]?.domain || "";
    slide.querySelector(".browser-address span:last-child").textContent = projects[0]?.browserAddress || "";
  } else {
    projects.forEach((project, index) => assets.appendChild(lightboxLink(project, index ? "project-gallery-image" : "project-main-image", gallery, index === 0)));
  }

  if (slideCount > 1) {
    const previous = `${categoryIndex + 1}-${((slideIndex - 1 + slideCount) % slideCount) + 1}`;
    const next = `${categoryIndex + 1}-${((slideIndex + 1) % slideCount) + 1}`;
    slide.querySelector(".project-navigation")?.appendChild(navigationButtons({ previous, next }));
  }
  return slide;
}

function buildCategory(category, projects, index) {
  const card = document.querySelector("#category-card-template").content.firstElementChild.cloneNode(true);
  card.querySelector(".project-type").textContent = category.verb || "";
  card.querySelector("h3").textContent = category.title || "";
  card.querySelector(".project-description").textContent = category.description || "";
  ...
  projects.filter((project) => project.image).forEach((project) => {
  ...
  slides.forEach(([layout, groupedProjects], slideIndex) => {
    card.querySelector(".project-slides").appendChild(buildSlide(layout, groupedProjects, index, slideIndex, slides.length));
  });
  return card;
}
  ...
  document.querySelectorAll("[data-grouped-project]").forEach((card) => {
    ...
    card.querySelectorAll("[data-project-switch]").forEach((button) => {
      button.addEventListener("click", (event) => {
        ...
        slides.forEach((slide) => { slide.hidden = slide.dataset.projectSlide !== button.dataset.projectSwitch; });
      });
    });
    ...
    const visibleImage = () => card.querySelector("[data-project-slide]:not([hidden]) .glightbox") || firstImage;
    card.addEventListener("click", (event) => {
      if (!event.target.closest("[data-project-switch], .glightbox")) visibleImage().click();
    });
    card.addEventListener("keydown", (event) => {
      if (event.key === "Enter" || event.key === " ") { event.preventDefault(); visibleImage().click(); }
    });
  });
}
...
async function loadCollections() {
  ...
  categories.forEach((category, index) => {
    const related = projects.filter((project) => project.classification?.category === category.title);
...
Promise.all([loadPages(), loadCollections()]).catch((error) => console.warn("Le contenu Decap n’a pas pu être chargé.", error));
